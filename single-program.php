                ...
                <section class="program-meta">
                    ...
                    <ul class="list-group">
                        ...
                        foreach ( $fields as $key => $label ) {
                            $value = get_post_meta( get_the_ID(), '_renita_program_' . $key, true );
                            if ( $value ) {
                                echo '<li class="list-group-item"><strong>' . esc_html( $label ) . ':</strong> ' . esc_html( $value ) . '</li>';
                            }
                        }

                        $college_name = get_post_meta( get_the_ID(), '_renita_program_college_name', true );
                        if ( $college_name ) {
                            echo '<li class="list-group-item"><strong>' . esc_html__( 'الكلية', 'renita' ) . ':</strong> ' . esc_html( $college_name ) . '</li>';
                        }

                        if ( ! empty( $department_terms ) ) {
                            echo '<li class="list-group-item"><strong>' . esc_html__( 'القسم الأكاديمي', 'renita' ) . ':</strong> ' . esc_html( $department_terms[0]->name ) . '</li>';
                        }

                        $modules = get_post_meta( get_the_ID(), '_renita_program_modules', true );
                        if ( $modules ) {
