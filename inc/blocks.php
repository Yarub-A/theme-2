...
add_action( 'init', function () {
    register_block_type( 'renita/news-grid', [
        ...
        'category'        => 'widgets',
        'icon'            => 'grid-view',
        'editor_script'   => 'renita-blocks-editor',
    ] );
...
function renita_render_quick_links( $attributes ): string {
    ...
    if ( empty( $links ) ) {
        $lang    = function_exists( 'pll_current_language' ) ? pll_current_language() : ( renita_is_rtl() ? 'ar' : 'en' );
        $links = [
            [
                'title' => renita_get_theme_string( 'apply_now', $lang ),
                'url'   => renita_get_page_link_by_path( 'admissions/requirements', $lang ) ?: home_url( '/admissions/requirements' ),
            ],
            [
                'title' => renita_localized_text( 'القبول والتسجيل', 'Admissions' ),
                'url'   => renita_get_page_link_by_path( 'admissions', $lang ) ?: home_url( '/admissions' ),
            ],
            [
                'title' => renita_get_theme_string( 'research_innovation', $lang ),
                'url'   => renita_get_page_link_by_path( 'research', $lang ) ?: home_url( '/research' ),
            ],
            [
                'title' => renita_localized_text( 'زيارة الحرم الجامعي', 'Visit Campus' ),
                'url'   => renita_get_page_link_by_path( 'contact', $lang ) ?: home_url( '/contact' ),
            ],
        ];
    }
